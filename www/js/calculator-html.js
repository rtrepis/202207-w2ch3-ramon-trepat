...
const start = () => {
    ...
    oneTd.onclick = () => {clickNumber(oneTd.innerHTML)}
    twoTd.onclick = () => {clickNumber(twoTd.innerHTML)}
    threeTd.onclick = () => {clickNumber(threeTd.innerHTML)}
    fourTd.onclick = () => {clickNumber(fourTd.innerHTML)}
    fiveTd.onclick = () => {clickNumber(fiveTd.innerHTML)}
    sixTd.onclick = () => {clickNumber(sixTd.innerHTML)}
    sevenTd.onclick = () => {clickNumber(sevenTd.innerHTML)}
    eightTd.onclick = () => {clickNumber(eightTd.innerHTML)}
    nineTd.onclick = () => {clickNumber(nineTd.innerHTML)}
    zeroTd.onclick = () => {if(screenTr.innerHTML > 0){clickNumber(zeroTd.innerHTML)}}
    commaTd.onclick = () => { if(screenTr.innerHTML.indexOf(',') > 0){}else{clickNumber(commaTd.innerHTML)}
    }

    divideTd.onclick = () => { clickOperator('divide', divideTd)}
    multiplyTd.onclick = () => { clickOperator('multiply', multiplyTd)}
    substracTd.onclick = () => { clickOperator('substrac', substracTd)}
    addTd.onclick = () => { clickOperator('add', addTd)}
    ...
    const clickNumber = (numberIs) => {
        ...
        if (isClean){
            ...
            screenTr.innerHTML = screenTr.innerHTML + numberIs;
            isClean = false;
        }
