...
do {
  askUser = prompt(
    'Introdueix els valors a operar o presiona "esc" per finalitzar'
  );
  if (askUser) {
    // si no faig aquest if --> trim em fa error al ser un camp buit
    if (askUser.trim() === "") {
      alert("Camp buït");
...
if (numsCalc.length === 0) {
  ...
  console.log(
    `L'arrel quadrada de ${numsCalc[0]} és : ${Math.sqrt(numsCalc[0]).toFixed(
      2
    )}`
  );
} else {
  const sum = numsCalc[0] + numsCalc[1];
  console.log(`La suma de ${numsCalc[0]} i ${numsCalc[1]} és : ${sum}`);
  const rest = numsCalc[0] - numsCalc[1];
  console.log(`La resta de ${numsCalc[0]} i ${numsCalc[1]} és : ${rest}`);
  const mult = numsCalc[0] * numsCalc[1];
  console.log(
    `La multiplicació de ${numsCalc[0]} i ${numsCalc[1]} és : ${mult}`
  );
  const div = numsCalc[0] / numsCalc[1];
  console.log(
    `La divisió de ${numsCalc[0]} i ${numsCalc[1]} és : ${div.toFixed(2)}`
  );
}
