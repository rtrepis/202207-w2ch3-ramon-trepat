...
const showRanking = () => {
  ...
  ranking.forEach(player => console.log(`El ${player.name}  -----  te -------- ${player.points}`));
};
...
newCardboard();
while (!askYesOrNot('Voleu aquesta taula? Yes/Not') ){newCardboard();} 
newTurn();
}
...
for(u=0; u<cardboard.length ; u++){
  ...
  for (i=0; i< cardboard[u].length; i++){
    ...
    if (linea > 4 && lineaPossible ){console.log("Linea !!!" ); lineaPossible = false; };
  };
};
...
console.table(cardboard);
if (replay) { askReturnPlay()}else{askNewTurn()}
};
