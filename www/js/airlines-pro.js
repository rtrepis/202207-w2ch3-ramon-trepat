...
console.log(`${userName} el cost mitjà del vols és de : ${(allCost/flights.length).toFixed(2)}€ dels cuals ${allScale} fant escala i els últims destins del dia son : ${lastFlightsTxt}.`);


//// Airlines PRO ///

var userType = askTwoOptions('Indica categoria d\'usuari? Admin/User', 'admin', 'user');
...
function userTasks(){
    ...
    const actionUser = askTwoOptions('Quina acció vols executar Buscar o Comprar ? B / C', 'b', 'c');
...
function adminTasks(){
    ...
    const actionAdmin = askTwoOptions('Quina acció vols executar Crear o Esborrar ? C / E', 'c', 'e');
    ...
    if (actionAdmin === 'c'){
        ...
        printFlights();
        const eraseflight = askNumber('Introduïu la id del vol ha esborrar:', 0, 9999);
...
function printFlights(){
    return flights.forEach(flight => { console.log(`id: ${flight.id} - Destí: ${flight.to} - Origen: ${flight.from} - Cost: ${flight.cost}€ - Escales: ${flight.scale}`) });
}
    ...
    do{
        ...
        if(answer){
            ...
            switch (answer.toLowerCase()){
                ...
                case 'not' : answer = false; break;
                default : alert(answer + ' no es una opció carrecta');
            };
        }else{
            alert('Camp buit');
        };
